#!/usr/bin/env python3
"""
Convert .docx files to .md with proper formatting preservation:
...
class DocxToMarkdownConverter:
    ...
    def convert_to_markdown(self, content_elements, doc_name):
        """Convert content elements to markdown format with proper formatting"""
        markdown_lines = []
        ...
        for i, element in enumerate(content_elements):
            # Check if we need to reset numbering (non-list element between lists)
            if prev_element_type == 'numbered_list' and element['type'] != 'numbered_list':
                # A non-list element appears, which should reset numbering
                list_counters = {}
